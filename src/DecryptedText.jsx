...
export default function DecryptedText({
  ...
  const shouldStartEncrypted = animateOn === 'click' || animateOn === 'load';
  const [displayText, setDisplayText] = useState(text);
  const [isAnimating, setIsAnimating] = useState(false);
  const [revealedIndices, setRevealedIndices] = useState(new Set());
  const [hasAnimated, setHasAnimated] = useState(false);
  const [isDecrypted, setIsDecrypted] = useState(!shouldStartEncrypted);
  ...
  const fillAllIndices = useCallback(() => {
    const s = new Set();
    for (let i = 0; i < text.length; i += 1) s.add(i);
    return s;
  }, [text]);
  ...
  useEffect(() => {
    if (animateOn === 'click' || animateOn === 'load') {
      encryptInstantly();
    } else {
      setDisplayText(text);
      setIsDecrypted(true);
  ...
  useEffect(() => {
    if (animateOn !== 'load' || hasAnimated) return undefined;
